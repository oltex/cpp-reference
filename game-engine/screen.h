...
namespace engine {
	...
	public:
		inline explicit screen(window::window& window) noexcept
			: _device(graphic::instance().get_device()), _context(graphic::instance().get_context()) {
	...
	public:
		inline void set_render_traget_view(void) noexcept {
			_context.OMSetRenderTargets(1, &_render_target_view, _depth_stencil_view);
		}
		inline void clear_render_target_view(void) noexcept {
			_context.ClearRenderTargetView(_render_target_view, _color);
		}
		inline void clear_dept_stencil_view(void) noexcept {
