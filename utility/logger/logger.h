...
namespace utility {
	...
	public:
		...
		inline void log_message(std::string_view const key, wchar_t const* const format, ...) noexcept {
			if (0 != _output && level_ >= _level) {
				...
				if (static_cast<unsigned char>(output::file) & _output) {
					...
					iter._lock.enter();
					{
						system_component::file file(iter._path.data(), FILE_WRITE_DATA, FILE_SHARE_READ, OPEN_EXISTING, FILE_FLAG_WRITE_THROUGH);
						file.set_pointer_ex(LARGE_INTEGER{ 0 }, FILE_END);
						file.write(buffer, static_cast<unsigned long>(length) * 2);
					}
					iter._lock.leave();
				}
			}
		...
		inline void create_file(std::string_view key, std::wstring_view path) noexcept {
			auto& iter = _logged.emplace(std::piecewise_construct, std::forward_as_tuple(key), std::forward_as_tuple(path)).first->second;
			unsigned long attribute = system_component::file::get_attribute(iter._path.data());
			if (attribute == INVALID_FILE_ATTRIBUTES || (attribute & FILE_ATTRIBUTE_DIRECTORY)) {
				for (size_type index = 0; index < path.size(); ++index) {
					if (L'\\' == path[index]) {
						...
						system_component::file::create_directory(iter._path.data());
						iter._path[index] = L'\\';
					}
				}
				system_component::file file(iter._path.data(), FILE_WRITE_DATA, FILE_SHARE_READ, OPEN_ALWAYS, FILE_FLAG_WRITE_THROUGH);
...
#define log(level, key, format, ...)\
{\
	auto& logger = utility::logger::instance();\
	if(utility::level >= logger.get_level()) {\
		logger.log_message<utility::level>(key, format, ##__VA_ARGS__);\
	}\
}\
