...
public:
	...
	inline void zoom(POINT const& point, float const offset) noexcept {
		POINT pos = camrea_to_client(point);
		_zoom += offset;
		if (_zoom < 0.1f)
			_zoom = 0.1f;
		POINT pos2 = camrea_to_client(point);

		_position.x += pos.x - pos2.x;
		_position.y += pos.y - pos2.y;
	};
public:
	inline POINT clinet_to_camrea(POINT const position)  const noexcept {
		POINT result;
	...
	inline RECT clinet_to_camrea(RECT const rect) const noexcept {
		RECT result;
	...
	inline POINT camrea_to_client(POINT const position) const noexcept {
		POINT result;
