	...
	inline void push(int value) noexcept {
		...
		for (;;) {
			...
			if (nullptr == _InterlockedCompareExchangePointer(reinterpret_cast<void* volatile*>(&next), (void*)current, nullptr)) {
				...
				_InterlockedCompareExchange(reinterpret_cast<unsigned long long volatile*>(&_tail), change, tail);
				//{
...
private:
	...
	library::data_structure::lockfree::object_pool<node> _object_pool;
