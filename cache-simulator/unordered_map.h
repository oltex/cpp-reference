...
template<typename _Kty, typename type, class hash = hash<_Kty>>
class unordered_map final {
	using pair = pair<_Kty, type>;
...
public:
	...
	inline void insert(_Kty const& key, universal&& value) const noexcept {
		emplace(key, std::forward<universal>(value));
	}
	template<typename... argument>
	inline auto emplace(_Kty const& key, argument&&... arg) noexcept -> iterator {
		auto iter = find(key);
		if (iter != _list.end())
			return iter;
	...
	inline auto operator[](_Kty const& key) noexcept -> type& {
