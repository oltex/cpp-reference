...
namespace example {
	inline void ring_buffer(void) noexcept {
		srand(static_cast<unsigned int>(6530985434));
		library::ring_buffer ring;
		...
		for (;;) {
			...
			if (ring.remain() >= push) {
				...
				ring.push(str + index, static_cast<unsigned int>(size));
				index = (index + size) % 120;
				ring.push(str + index, static_cast<unsigned int>(push - size));
