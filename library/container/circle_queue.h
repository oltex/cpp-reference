...
namespace library {
	...
	class circle_queue final {
		...
		type* _array;
	public:
		inline explicit circle_queue(void) noexcept
			: _capacity(1), _front(0), _rear(0), _array(library::allocate<type>(1)) {
		};
		inline explicit circle_queue(circle_queue const&) noexcept = delete;
		inline explicit circle_queue(circle_queue&& rhs) noexcept
			: circle_queue() {
			swap(rhs);
		};
