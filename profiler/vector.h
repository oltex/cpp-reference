#pragma once
#include <initializer_list>
#include <utility>
#include <stdlib.h>

template<typename _Ty>
class vector {
public:
	using iterator = _Ty*;
public:
	inline explicit vector(void) noexcept = default;
	inline explicit vector(std::initializer_list<_Ty> const& list) noexcept {
		reserve(list.size());
		for (auto& iter : list)
			emplace_back(iter);
	}
	inline explicit vector(iterator const& begin, iterator const& end) noexcept {
		reserve(end - begin);
		for (auto iter = begin; iter != end; ++iter)
			emplace_back(*iter);
	}
	inline explicit vector(vector const& rhs) noexcept
		: vector(rhs.begin(), rhs.end()) {
	}
	inline explicit vector(vector&& rhs) noexcept {
		swap(rhs);
	}
	inline ~vector(void) noexcept {
		clear();
		free(_arr);
	}
public:
	template<typename _Uy>
	inline void push_back(_Uy&& value) noexcept {
		emplace_back(std::forward<_Uy>(value));
	}
	template<typename... _Arg>
	inline auto emplace_back(_Arg&&... argument) noexcept -> _Ty& {
		if (_size >= _capacity) {
			_capacity = static_cast<size_t>(_capacity * 1.5f);
			if (_size >= _capacity)
				_capacity++;
			reserve(_capacity);
		}
		_Ty* value = new(_arr + _size) _Ty(std::forward<_Arg>(argument)...);
		++_size;
		return *value;
	}
	inline void pop_back(void) noexcept {
		--_size;
		_arr[_size].~_Ty();
	}
public:
	inline auto front(void) const noexcept ->_Ty& {
		return _arr[0];
	}
	inline auto back(void) const noexcept ->_Ty& {
		return _arr[_size - 1];
	}
	inline auto operator[](size_t const idx) const noexcept ->_Ty& {
		return _arr[idx];
	}
public:
	inline auto begin(void) const noexcept -> iterator {
		return _arr;
	}
	inline auto end(void) const noexcept -> iterator {
		return _arr + _size;
	}
public:
	inline auto size(void) const noexcept -> size_t {
		return _size;
	}
	inline bool empty(void) const noexcept {
		return 0 == _size;
	}
	inline auto capacity(void) const noexcept {
		return _capacity;
	}
	inline void reserve(size_t const& capacity) noexcept {
		if (_size > capacity)
			return;
		_capacity = capacity;
#pragma warning(suppress: 6308)
		_arr = static_cast<_Ty*>(realloc(_arr, sizeof(_Ty) * _capacity));
	}
	inline void resize(size_t const& size) noexcept {
		//if (_capacity < size)
		//	reserve(size);
		//for (; _size < size; ++_size)
		//	new(_arr + _size) _Ty(value);
	}
	inline void clear(void) noexcept {
		while (0 != _size)
			pop_back();
	}
	inline void swap(vector& rhs) noexcept {
		_Ty* arr = _arr;
		_arr = rhs._arr;
		rhs._arr = arr;

		size_t size = _size;
		_size = rhs._size;
		rhs._size = size;

		size_t capacity = _capacity;
		_capacity = rhs._capacity;
		rhs._capacity = capacity;
	}
	inline void assign(size_t const size, _Ty const& value) noexcept {
		clear();
		if (_capacity < size)
			reserve(size);
		for (; _size < size; ++_size)
			new(_arr + _size) _Ty(value);
	}
protected:
	_Ty* _arr = nullptr;
	size_t _size = 0;
	size_t _capacity = 0;
};